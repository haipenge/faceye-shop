...
<div class="page-head">
	<h2>
		...
		<a class="btn btn-primary" href="<c:url value="/lbs/geoLibrary/input"/>"> <fmt:message
...
<div class="cl-mcont">
	...
	<c:import url="/component/core/template/msg/msg.jsp" />
		<div class="content">
			<form action="<c:url value="/lbs/geoLibrary/home"/>" method="post" role="form"
 ...
 <!--@generate-entity-jsp-property-desc@-->
					...
					<tbody>
						<c:forEach items="${page.content}" var="geoLibrary">
							<tr>
							   <td><input type="checkbox" name="check-single" value="${geoLibrary.id}"></td>
								<td>${geoLibrary.name}</td>   
 <td>${geoLibrary.orderIndex}</td>   
 <td>${geoLibrary.code}</td>   
 <td>${geoLibrary.level}</td>   
 <!--@generate-entity-jsp-property-value@-->
								<td><a href="<c:url value="/lbs/geoLibrary/detail/${geoLibrary.id}"/>">
										<fmt:message key="global.view"></fmt:message>
								</a></td>
								<td><a href="<c:url value="/lbs/geoLibrary/edit/${geoLibrary.id}"/>">
										<fmt:message key="global.edit"></fmt:message>
								</a></td>
								<td><a href="<c:url value="/lbs/geoLibrary/remove/${geoLibrary.id}"/>">
			...
			<f:page page="${page}" url="/lbs/geoLibrary/home" params="<%=request.getParameterMap()%>" />
		</div>
	</div>
</div>
