...
//		alert(filename);
	...
	if (window.location.hostname === 'localhost' || window.location.hostname === 'm.faceye.com') {
		// Demo settings:
...
//				alert(productId);
		...
		if (url) {
			...
			productId = uArray[uArray.length - 1];
		}
		// 验证
		var regex = /^[1-9]\d*$/;
		var isNumber = false;
		isNumber = regex.test(productId);
		// alert(docId);

		if (isNumber) {
			$.ajax({
				url : '/product/image/getImagesByProduct',
				type : 'post',
				dataType : 'json',
				data : {
					productId : productId
