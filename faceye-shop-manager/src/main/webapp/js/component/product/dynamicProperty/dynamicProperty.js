...
var DynamicProperty = {
	init : function() {
		...
		$('select[name="category.id"]').change(function() {
			DynamicProperty.onCategorySelect(this);
		});
		...
		if (checkedIds != '') {
			$.ajax({
				...
				success : function(data, textStatux, xhr) {
					var type=data.result=='true' ? '' : 'warning';
					var msg = new Msg({
						msg : data.msg,
						type: type
					});
	...
	onCategorySelect : function(el) {
		...
		if (categoryId && categoryId != 0) {
			$('#isShare1').prop('checked', false);
			$('#isShare2').prop('checked', true);
			$('#isShare1').parent().parent().hide();
		} else {
			$('#isShare1').parent().parent().show();
		}
	}
